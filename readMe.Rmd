...
library(ggplot2)
ggplot(data=commute, aes(x=day, y=duration, size=mpg, color=destination))+geom_point(alpha=.5,position = position_jitter(w = 0.1, h = 0))+facet_grid(.~destination)+scale_size_continuous(range = c(3,10))
ggplot(data=commute, aes(x=mpg,   y=duration, color=destination))+geom_point(alpha=.8,size=5)+facet_grid(.~destination)
ggplot(data=commute, aes(x=mpg,   y=duration, color=destination))+geom_point(alpha=.8,size=5)
```

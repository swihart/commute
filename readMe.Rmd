...
# breaks_mpg<- sort(commute$mpg)[c(1, round(n_rec*.2), round(n_rec*.4), round(n_rec*.6), round(n_rec*.8), n_rec)]
...
#   scale_y_continuous(breaks=seq(range(commute$duration, na.rm=TRUE)[1],range(commute$duration, na.rm=TRUE)[2], length=6))+ ## explicitly put min/max for y axis 
